...
Core 負責共享的請求模型、驗證、dry-run 規劃、執行階段工作階段協調、事件/結果類型、控制面介面、設定檔 metadata，以及 Keysight 34460A/34461A 擷取執行階段的安全規則。它會隨單一的 `keysight-logger` 發行套件一起發布，同時保留 `keysight_logger_core` 的 import 邊界。
...
CLI 與 WebUI 元件各自負責自己的命令列、網頁、包裝器與序列化層。Core 不得 import `keysight_logger_cli` 或 `keysight_logger_webui`。
