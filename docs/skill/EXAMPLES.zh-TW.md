...
3. 先執行 dry-run。
4. 使用 documented software-trigger subprocess orchestration pattern，以 finite
   bound 執行 simulator mode。
...
2. 在使用 CLI help 前，先從 contracts 確認 exact CLI flag spelling。
3. 先執行 dry-run。
4. 使用 SIM::34461A 與 finite bound 執行 simulator validation，並使用 documented
   software-trigger subprocess orchestration pattern。
5. 將 worker 作為 observable subprocess 啟動，stream stdout JSONL 直到 ready
   event；不要使用 PowerShell Start-Process 或 cmd /c start /B 這類 detached
   shell launch，除非 repository 有文件化。
