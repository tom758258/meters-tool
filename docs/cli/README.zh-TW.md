...
- DC 電流、DC 電壓、DCV 比率、AC 電流、AC 電壓以及 2 線或 4 線電阻量測記錄。
...
- 選用的量測控制項：量測類型、自動範圍、手動範圍、DCV 輸入阻抗、包含 `once` 在內的自動歸零（Auto Zero）、NPLC、AC 頻寬、電流端子選擇、硬體觸發延遲、硬體觸發斜率與 VM Comp 斜率。
- 每筆擷取樣本後立即排空（flush）CSV。

重要限制：

- 本專案目前專注於 Keysight 34461A 的電流、電壓、DCV 比率以及 2 線或 4 線電阻記錄。
- AC 模式透過 `--ac-bandwidth-hz` 公開 34461A 的 `3`、`20` 和 `200` Hz 頻寬設定。在實際投入生產使用前，請使用操作者提供的 VISA 資源執行低風險的實際資源煙霧測試，並將 CLI 資料列與 34461A 前面板讀數進行對比。
- `--nplc` 和 `--auto-zero` 是 DC/電阻控制項。AC 電流和 AC 電壓僅接受中性預設值 `--nplc 1.0`；任何其他 NPLC 值都將被拒絕，因為 AC 模式不會寫入 NPLC SCPI。AC 模式也不會寫入 Auto Zero SCPI 指令。
...
- 透過 USB 或 LAN 對 VISA 可見的 Keysight 34461A。
...
安裝後，使用 `keysight-logger` 主控台指令來執行專案命令：

```powershell
.\.venv\Scripts\keysight-logger.exe <command> [options]
```

`.venv\Scripts\keysight-logger.exe` 是安裝時產生的，並非受版本控制的專案檔案。如果遺失，請重新執行 `uv pip install -e ".[all,dev]"`。如果 PowerShell 因執行原則限制而阻擋啟用，請繼續使用本指南中顯示的明確 `.\.venv\Scripts\...` 命令。
...
安裝的 `.venv\Scripts\keysight-logger.exe` 是一個 virtualenv 主控台包裝器。對於沒有專案環境的機器，它並非獨立的執行檔。

若要建置選用的獨立主控台 exe，請在已安裝 `keysight-logger` 的環境中使用 PyInstaller。PyInstaller 是本機版本建置工具，而非 CLI 執行階段相依套件，因此在全新機器上重新建置之前，請將其安裝到 venv 中：
...
5. 如果規劃看起來正確，請執行最小限度的實際煙霧測試。包裝器會先執行 preflight，列印規劃的儀器狀態變更，並在實際擷取前要求互動式的 Enter 確認：
...
若要獲得更廣泛的實際覆蓋範圍，請在最小套件通過後使用 `-Suite basic`。該套件涵蓋即時量測和軟體觸發路徑。僅當操作者可以安全提供所需的外接觸發邊緣時，才使用 `-Suite external`。僅當同時需要基本和外接覆蓋時，才使用 `-Suite full`。
...
| `scripts\live-cli-check.ps1` | 實體硬體（除非設定了 `-PlanOnly`） | 執行實際包裝器規劃，並在互動確認後，針對明確的 `-Resource` 執行有限的實際煙霧測試案例。 |
...
| 選項 | 說明 |
| --- | --- |
| `--version` | 列印 `keysight-logger <package-version>`並退出，不需要子指令。 |
...
| `--measurement current-dc\|voltage-dc\|voltage-dc-ratio\|current-ac\|voltage-ac\|resistance-2w\|resistance-4w` | 否 | `current-dc` | 量測類型。 |
| `--nplc VALUE` | 否 | `1.0` | 用於 DC 電流、DC 電壓、DCV 比率和電阻的電源線週期積分時間。DC/電阻/比率的允許值：`0.02`、`0.2`、`1`、`10`、`100`。AC 電流和 AC 電壓僅接受中性預設值 `1.0`；除非您刻意傳送 `1.0`，否則 AC 模式請省略 `--nplc`。 |
| `--auto-zero on\|off\|once` | 否 | `on` | 支援量測的自動歸零（Auto Zero）。`once` 在 DC 電流、DC 電壓和 2 線電阻下有效。DCV 比率僅接受預設/啟用（on）行為且不寫入 Auto Zero SCPI；4 線電阻和 AC 量測則將 Auto Zero 交給儀器處理。 |
| `--auto-range on\|off` | 否 | `on` | 啟用或停用自動範圍（Auto Range）。 |
| `--range VALUE` | 當 `--auto-range off` 時必要 | 無 | 所選量測的手動範圍。電流為安培，電壓為伏特，電阻為歐姆。 |
| `--current-range VALUE` | 僅限 DC 電流 | 無 | 與 `current-dc` 的 `--range` 相容的別名。不可與 `--range` 同時使用；在 AC 電流、電壓和電阻量測下無效。 |
| `--ac-bandwidth-hz 3\|20\|200` | 僅限 AC | 無 | 用於 AC 電流和 AC 電壓的 AC 頻寬/濾波器設定。省略以保持儀器預設/目前設定不變。 |
...
`--measurement` 預設為 `current-dc`，因此現有的電流量測記錄指令不需要特別指定。新指令應偏好使用 `--range`；`--current-range` 繼續適用於現有的 DC 電流腳本。對於 `current-ac`、`voltage-dc`、`voltage-dc-ratio`、`voltage-ac`、`resistance-2w` 和 `resistance-4w` 請使用 `--range`；`--current-range` 在這些量測下會被拒絕。
`--dcv-input-impedance` 僅在 `--measurement voltage-dc` 或 `--measurement voltage-dc-ratio` 時有效。使用 `default` 以保持儀器目前的 Input Z設定不變，`10m` 強制為 10 MOhm，或 `auto` 啟用 34461A 的 Auto Input Z 行為。當 Auto 作用於較低 DC 電壓範圍時，儀器可能會顯示 HighZ。
...
| `--auto-range` | `on` 或 `off` |
| `--ac-bandwidth-hz` | `3`, `20` 或 `200`（僅限 AC 電流/電壓） |
...
這些範例依照實用驗證路徑排序：先確認作用中資源，然後執行單一樣本煙霧檢查，最後使用符合實驗設計的觸發模式。下方顯示的 USB 資源是專案驗證時使用的 34461A；請替換為適合您儀器和測試執行的資源字串與 CSV 路徑。
...
2. 以 `--trigger-mode immediate` 與 `--max-samples 1` 執行單一樣本的電流、電壓或電阻煙霧測試。
...
在進行無人值守擷取前，請使用操作者提供的 Keysight 34461A VISA 資源來驗證工作流程。從即時模式、自動範圍開啟以及 `--max-samples 1` 開始，然後擴展至預期的量測、觸發模式與緩衝模式。對於 AC 電流與 AC 電壓，請在煙霧測試期間將 CLI CSV 資料列與 34461A 前面板讀數進行對比。
...
實際 10 A 端子煙霧檢查。僅在操作者確認電流路徑對 10 A 輸入端子和預期電流安全後才可執行：
...
對於這些檢查，請確認前面板 Input Z 狀態是否符合預期變更：`auto` 應選擇自動並在較低的 DC 電壓範圍顯示 HighZ，而 `10m` 應選擇固定的 10 MOhm。這兩個 DCV Input Z 煙霧檢查在 `v1.0.0-cli` 基準前於真實 34461A 上回報正常。
...
AC 電流與 AC 電壓使用與其他純量（scalar）量測相同的觸發/讀取流程。它們設定 34461A 的 AC 功能、自動/手動範圍與選用的 AC 頻寬。對於 AC 量測，CLI 不會寫入 NPLC 或 Auto Zero SCPI。在實際煙霧測試時，請從即時模式、自動範圍開啟與 `--max-samples 1` 開始，然後對照 CLI CSV 欄位與 34461A 前面板讀數。
...
對於 AC 資料列，預期 `measurement_type=voltage_ac` 且 `unit=V`，或 `measurement_type=current_ac` 且 `unit=A`。在煙霧測試期間，請對照 CSV 資料與前面板讀數，確認無誤後再使用較長期的擷取。
...
對於電阻列，預期 CSV 中的 `measurement_type=resistance_2w` 且 `unit=Ohm`。量測值應對於連接的電阻或開路狀態是合理的。`resistance-2w` 也支援現有的軟體、計時器、外接和自訂/緩衝模式；在生產環境使用這些觸發路徑前，請先執行專注的實體儀器檢查。

### 4 線式電阻煙霧測試

這些指令使用與其他純量（scalar）量測相同的觸發/讀取流程，但 4 線式 SCPI 功能為 `FRES`。CLI 不會寫入 `FRES:ZERO:AUTO`，因為 34461A 在內部處理 4 線電阻的 Auto Zero。移除 `FRES:ZERO:AUTO` 後，這些煙霧測試在真實的 34461A 上回報正常。
...
`--nplc 10.0` 搭配 `--auto-zero on` 對於 DC/電阻量測而言十分緩慢。若需要較快的外接觸發步調，請考慮使用較低的 NPLC 和關閉 Auto Zero（例如 `--nplc 1.0 --auto-zero off`）。AC 量測不使用 Auto Zero，且僅接受中性 `--nplc 1.0`。
...
像是 `mA`, `mV`, `kOhm`, `MOhm` 的顯示字首僅用於主控台。CSV 資料列會繼續以量測的基本單位（`A`, `V`, `ratio` 或 `Ohm`）儲存原始數值。自訂/緩衝模式可能會一次排空多個讀數；主控台狀態會顯示該次排空批次中的最後一個樣本。
...
| `measurement_type` | 選取的量測類型，例如 `current_dc`、`voltage_dc`、`voltage_dc_ratio`、`current_ac`、`voltage_ac`、`resistance_2w` 或 `resistance_4w`。 |
| `value` | 量測數值。 |
| `unit` | 單位，電流為 `A`，電壓為 `V`，DCV 比率為 `ratio`，電阻為 `Ohm`。 |
...
- 如果缺少 `.\.venv\Scripts\keysight-logger.exe`，請重新執行 `uv pip install -e ".[all,dev]"`。主控台指令是安裝產物，而非版本控制下的專案檔案。
- 如果 PowerShell 啟用受阻，請繼續使用明確的 `.\.venv\Scripts\python.exe` 或 `.\.venv\Scripts\keysight-logger.exe` 指令，而非啟用虛擬環境。
