...
適用於 Keysight 34461A 的 CLI 優先 Python 記錄器，支援透過 VISA 進行 DC/AC 電流、DC/AC 電壓、DCV 比率、頻率、週期以及 2 線式或 4 線式電阻量測。
...
- 選用的量測控制項：量測類型、自動範圍、手動範圍、DCV 輸入阻抗、包含 `once` 在內的自動歸零（Auto Zero）、NPLC、AC 頻寬/濾波器、頻率/週期閘門時間、僅限頻率的 timeout、電流端子選擇、硬體觸發延遲、硬體觸發斜率與 VM Comp 斜率。
- 每筆擷取樣本後立即排空（flush）CSV。

重要限制：

- 本專案目前專注於 Keysight 34461A 的電流、電壓、DCV 比率、頻率、週期以及 2 線或 4 線電阻記錄。
...
| `--nplc VALUE` | 否 | `1.0` | 用於 DC 電流、DC 電壓、DCV 比率和電阻的電源線週期積分時間。DC/電阻/比率的允許值：`0.02`、`0.2`、`1`、`10`、`100`。AC、頻率和週期僅接受中性預設值 `1.0`；這些模式不會寫入 NPLC SCPI。 |
| `--auto-zero on\|off\|once` | 否 | `on` | 支援量測的自動歸零（Auto Zero）。`once` 在 DC 電流、DC 電壓和 2 線電阻下有效。DCV 比率僅接受預設/啟用（on）行為且不寫入 Auto Zero SCPI；4 線電阻、AC、頻率和週期量測則將 Auto Zero 交給儀器處理。 |
| `--auto-range on\|off` | 否 | `on` | 啟用或停用自動範圍（Auto Range）。 |
| `--range VALUE` | 當 `--auto-range off` 時必要 | 無 | 所選量測的手動範圍。電流為安培；電壓、頻率和週期輸入範圍為伏特；電阻為歐姆。 |
| `--current-range VALUE` | 僅限 DC 電流 | 無 | 與 `current-dc` 的 `--range` 相容的別名。不可與 `--range` 同時使用；在其他量測下無效。 |
| `--ac-bandwidth-hz 3\|20\|200` | 僅限 AC/頻率/週期 | 依量測而定 | AC 頻寬/濾波器設定。AC 電流/電壓省略時保持目前設定；頻率和週期省略時預設為 `20` Hz。 |
| `--gate-time-s 0.01\|0.1\|1` | 僅限頻率/週期 | `0.1` | 頻率/週期 aperture 或閘門時間，單位為秒。 |
| `--freq-period-timeout auto\|1s` | 僅限頻率 | `auto` | 使用自動頻率 timeout，或停用自動 timeout 以使用固定 1 秒行為。週期不支援此選項，也不會送出 timeout SCPI。 |
...
| `--auto-range` | `on` 或 `off` |
| `--ac-bandwidth-hz` | `3`, `20` 或 `200`（僅限 AC 電流/電壓、頻率、週期） |
...
| `frequency` | `0.1`, `1`, `10`, `100`, `750` V |
| `period` | `0.1`, `1`, `10`, `100`, `750` V |
...
- `--freq-period-timeout` 僅在 `frequency` 下有效；週期明確指定此選項時會在開啟 VISA 前被拒絕。
