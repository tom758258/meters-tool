...
For CI or strict lock validation, add `--locked` to the sync command. Normal
development follows the ordinary `uv sync` flow in the root README. If an
existing virtual environment is already synchronized but a console wrapper is
missing or project installation metadata needs to be rebuilt, use:
...
tracked project file. If it is missing, first rerun the ordinary
`uv sync --all-extras --link-mode=copy` flow from the root [README Install](../../README.md#install).
If the wrapper is still missing after ordinary sync, force uv to reinstall only
the project package:
...
This recreates the console wrapper without requiring pip. If PowerShell blocks
activation because of execution policy, keep using the explicit
`.venv\Scripts\...` commands shown in this guide.
...
- If `.\.venv\Scripts\meters-tool.exe` is missing, first rerun the ordinary
  [README Install](../../README.md#install) flow with
  `uv sync --all-extras --link-mode=copy`. If the wrapper is still missing after
  ordinary sync, use:
  ...
  This recreates the console wrapper without requiring pip.
