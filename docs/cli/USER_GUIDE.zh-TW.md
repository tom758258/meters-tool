...
在測量電流或 4 線電阻前，請確認輸入端子接線。對於 AC、頻率與週期模式，在將設定用於更長時間的採集之前，請先執行低風險的快速檢查，並比對 CSV 數值與前置面板讀數。
...
`--max-samples` 限制簡單執行的次數。在快速檢查與驗證期間使用它，使命令能自行停止。
...
每個擷取樣本都會寫成一筆 CSV 資料列。在執行快速檢查後，請檢查 CSV 以確認：
