...
本指南適用於取得已建置之 WebUI 啟動器，並使用它來記錄支援的 Keysight Truevolt DMM 量測數據的操作人員。本指南避開了開發人員細節，專注於標準的量測工作流程。
...
- `Device / Resource` (裝置/資源列)：儀器位址、最近一次掃描回應的即時資源、`Scan Device` 按鈕，以及 `Device options` 齒輪中的 `Expected model` 選擇器。此列預設展開，可收合成資源/型號摘要。
...
8. 除非您特別需要軟體或外部觸發，否則請將觸發模式保持在即時/預設模式 (immediate/default mode)。
9. 檢閱反白顯示的設定。
10. 點擊 `Start` (啟動)。
11. 觀察 `Captured` (已擷取)、`Status` (狀態) 及 `Live data` (即時數據)。
...
`Live resource` (即時資源) 顯示最後一次掃描回應的結果。請先用它確認是哪台儀器回應，再複製或選取資源以進行作業。當掃描辨識出支援的 34460A 或 34461A IDN 時，WebUI 可能會載入特定型號的選項以供顯示，同時保持 `Expected model` 在 `Auto-detect`。
...
`Expected model`（預期型號）是 `Device options` 中的選用檢查。Auto-detect 在 Start 時使用最新的 IDN preflight 來解析連接的儀器。只有當您希望 Start 讀取 IDN，且在連接的儀器不回報該支援型號就失敗時，才選擇 `Require 34460A` 或 `Require 34461A`。若要求 34460A，WebUI 會隱藏 10 A 電流範圍、電流端子選擇與外接觸發模式；自訂模式的讀數記憶體為 1000 筆。這些停用的控制項僅供指引。選取的型號是預期型號防護與顯示上下文；它不會覆寫偵測到的 IDN、解鎖另一台儀器的功能，或取代 Core 的安全檢查。
...
## 即時數據圖表縮放 (Live Data Chart Scale)

`Live data` 面板在 `Trend` 區域有圖表縮放控制項。這些設定僅影響瀏覽器圖表顯示。它們不影響儀器設定、SCPI 指令、CSV 輸出或記錄的數值。
...
`Manual span`（手動跨度）使用第一個數值樣本為中心，以及操作人員輸入的固定正數跨度。跨度使用原始量測單位：`V` 代表伏特，`A` 代表安培，`Ohm` 代表歐姆，`Hz` 代表赫茲，`s` 代表秒。對於電壓，`0.01` 代表 `0.01 V`，而不是 `0.01 mV`。如果第一個樣本是 `5.000 V` 且手動跨度是 `0.010 V`，則圖表顯示 `4.990 V` 到 `5.010 V`。超出 `baseline +/- span` 的數值會被裁切到圖表邊界。

`Range step`（範圍階梯）使用手動選取的 `Range` 作為圖表顯示跨度。僅在 `Auto Range` 關閉且選取手動範圍時可用。它不反映儀器的實際自動選擇範圍，因為在啟用自動範圍時 WebUI 無法得知該硬體範圍。如果第一個樣本是 `5.000 V` 且選取的手動 Range 是 `0.010 V`，則圖表顯示 `4.990 V` 到 `5.010 V`。超出 `baseline +/- selected Range` 的數值可能會被裁切到圖表邊界。與其他圖表縮放模式一樣，Range step 僅影響瀏覽器圖表顯示，不會改變儀器設定、SCPI 指令、CSV 輸出或記錄的數值。
...
### 行動裝置寬度（約 390 像素）或桌面寬度（約 1280 像素）顯示不正常

請參閱 [WebUI 變更規則](web-ui-change-rules.md) 以取得詳細的版面規範。
...
請確認作業已擷取到樣本並確認狀態輪詢正常。此面板僅使用 Core 樣本事件，不獨立查詢儀器或解析 CSV 檔案。
