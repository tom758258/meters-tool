...
當 `Expected model` 保持在 `Auto-detect` 時，WebUI 會在 Start 時透過新的 IDN preflight 自動辨識連接中的 34460A 或 34461A。只有在希望 Start 先讀取 IDN，並在不符時阻止啟動時，才選擇 `Require 34460A` 或 `Require 34461A`。若明確指定的型號不符，WebUI 會在 setup SCPI 之前提示實際找到的支援型號。
...
WebUI 使用電腦的預設系統 VISA 執行階段。它不提供 PyVISA backend 選擇器。只有在測試程序明確要求可選的 pyvisa-py 診斷時，才使用 CLI 的進階 `--visa-library` 選項。

`Live resource` (即時資源) 顯示最後一次掃描回應的結果。請先用它確認是哪台儀器回應，再複製或選取資源以進行作業。
