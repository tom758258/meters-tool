...
簡單的即時觸發與軟體觸發讀取，會使用 Core 的 `READ?` 路徑。硬體觸發的簡單讀取，則會在觸發配接器準備就緒 (arms) 並完成量測後，使用 Core 的 `FETC?` 路徑。
...
- 硬體觸發逾時是一種保護性的重新準備 (re-arm) 條件，其本身並非擷取錯誤。
...
後端會將瀏覽器傳送的資料 (payload) 轉換為 Core 的 `StartRequest`，透過 Core 進行驗證，建立配接器可見的狀態，並在背景工作執行緒 (worker) 上啟動 Core 的 `run_start_session()`。
...
停止動作會經由 Core 的控制面 (control plane) 進行路由。WebUI 程式碼絕不可直接關閉 VISA 控制代碼 (handles)、呼叫擷取引擎內部運作或重組清除順序。
...
瀏覽器端的趨勢圖表會將該次執行中的第一個數值樣本做為基準線 (baseline)，並在每次轉譯 (render) 時重新縮放，使得最大可見偏差對應到偏離中心線的四個網格步長。這僅影響圖表顯示，不影響原始樣本值、統計資料、CSV 輸出或 API 樣本 payload。
...
隱藏的控制項必須被停用，以免從未作用的模式中提交過時的數值。

頻率 (Frequency) 與週期 (Period) 的 payload 保留原始數值：AC 濾波器 `20 Hz` 會以 `ac_bandwidth_hz: 20` 傳送，閘門時間 (Gate Time) 為秒，且逾時 (Timeout) 為 `auto` 或 `1s`。即時數據會以 `Hz` 顯示頻率並以 `s` 顯示週期，且不進行自動單位縮放。
...
對於首次真實儀器的快速功能健檢 (smoke tests)，請使用低風險的即時 (immediate) 模式、啟用自動範圍 (Auto Range on)，以及較小有界限的樣本數。
...
未經使用者明確同意，請勿變更 `pyproject.toml` 中的根套件 metadata。套件名稱、版本、相依套件、主控台指令、建置系統、pytest/ruff/mypy 設定，以及 Core/CLI/WebUI 的擁有權劃分，皆屬於產品邊界決策。
...
實機驗證需要操作者提供 VISA 資源。在開始使用其他觸發模式或進行較長期的擷取之前，請先使用低風險的即時模式進行快速功能健檢，啟用自動範圍，並設定 `max_samples=1`。
...
- 頻率和週期顯示 AC 濾波器和閘門時間 (Gate Time)。頻率也會顯示逾時 (Timeout)；週期和其他量測模式會隱藏並停用它。
- 電流量測在支援處顯示電流端子。
- 變更觸發模式僅顯示和隱藏相關欄位。
...
針對真實儀器的快速功能健檢，除非操作者明確要求，否則請勿進行高風險的觸發實驗。請先以即時 (immediate) 模式、啟用自動範圍 (Auto Range on) 與 `max_samples=1` 開始。
...
- 在改變量測類型前先停止執行，並在每次採樣後檢查產生的 CSV 資料列。
...
- 此為預期行為。請使用 `Ctrl+C`，或透過 PID 停止監聽中的 `python.exe`。
...
- 嘗試手動輸入已知的 VISA 資源。
- 在進行真實擷取前，先以低風險的即時模式、啟用自動範圍與 `max_samples=1` 進行快速功能健檢。
...
- [WebUI 變更日誌](CHANGELOG.md)：套件版本發佈說明。
