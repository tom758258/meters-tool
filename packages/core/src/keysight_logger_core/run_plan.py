...
def _trigger_description(trigger_mode: str, request: StartRequest) -> str:
    ...
    if trigger_mode == "external":
        return (
            ...
            f"slope={str(request.hw_trigger_slope).upper()} delay_s={request.hw_trigger_delay_s:g}"
        )
    if trigger_mode == "immediate":
        return "immediate reads"
    if trigger_mode.endswith("-custom"):
        return (
            f"{trigger_mode} buffered capture "
            f"trigger_count={request.trigger_count} sample_count={request.sample_count}"
        )
    return trigger_mode


def _sample_limit_description(trigger_mode: str, request: StartRequest) -> str:
    if (
        trigger_mode.endswith("-custom")
        and request.trigger_count is not None
        and request.sample_count is not None
    ):
        return f"{request.trigger_count * request.sample_count} expected buffered readings"
    if request.max_samples is not None:
        return f"max_samples={request.max_samples}"
    return "unbounded until stop"


def _option_summary(request: StartRequest) -> dict[str, object]:
    ...
    if request.buffer_drain_size is not None:
        summary["buffer_drain_size"] = request.buffer_drain_size
    if request.allow_buffer_overflow_risk:
        summary["allow_buffer_overflow_risk"] = True
    if request.vm_comp_slope is not None:
        summary["vm_comp_slope"] = request.vm_comp_slope
    return summary
