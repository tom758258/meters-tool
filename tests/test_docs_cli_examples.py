...
RUNNABLE_MODULE_PATTERNS = (
    r".\.venv\Scripts\python.exe -m keysight_logger.cli ",
    "python -m keysight_logger.cli ",
)
...
def test_active_docs_prefer_console_script_for_runnable_cli_examples():
    ...
    forbidden_docs = [
        repo_root / "README.md",
        repo_root / "docs" / "hardware-test-plan.md",
        repo_root / "docs" / "project-plan.md",
    ]
...
def test_cli_guide_module_form_only_appears_as_explicit_alternative():
    ...
    assert module_lines == [
        r".\.venv\Scripts\python.exe -m keysight_logger.cli <command> [options]",
        r".\.venv\Scripts\python.exe -m keysight_logger.cli <command> [options]",
    ]
